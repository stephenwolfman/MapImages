...
$(document).ready(function () {
    ...
    $('.editLink').bind('click',function () { EditMapImage($(this).attr('mapimageid')); })
});
...
function EditMapImage(id) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $('#txtLong').val(result.Long);
            $('#txtDesc').val(result.Desc);
            $('#txtSentBy').val(result.SentBy);
            $('#txtComment').val(result.Comment);
            $('#txtImageUrl').val(result.ImageUrl);
            $('#txtVideoUrl').val(result.VideoURL);
            if ((result.ImageUrl != null || result.ImageUrl == '') && (result.VideoURL == null || result.VideoURL == '')) {
                $('#imgMapImage').attr('src', mapUrl + result.ImageUrl);
                $('#imgMapImage').show();
                $('#iVideo').hide();
            } else {
                $('#iVideo').attr('src', result.VideoURL);
                $('#iVideo').show();
                $('#imgMapImage').hide();
            }
            $('#btnMapImageSave').data('MapImageId', result.MapImageId);
            //Show edit
...
function UpdateMapImage(evt) {
    ...
    $.ajax({
        url: '/api/mapimageupdate/' + MapImageId,
        type: 'PUT',
